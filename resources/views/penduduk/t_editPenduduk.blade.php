{{-- modalEditDataPenduduk --}}
    <div class="modal fade" id="modalEditDataPenduduk" tabindex="-1" aria-labelledby="modalEditDataPendudukLabel" aria-hidden="true">
        <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
            <h5 class="modal-title" id="modalEditDataPendudukLabel">Edit Data Penduduk</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
            </button>
            </div>
            <form class="formEditPenduduk" method="post">
            @method('put')
            @csrf
            <div class="modal-body modalEditDataPenduduk" style="background-color: rgb(236, 236, 236)">
                ...
                <input autocomplete="off" class="form-control @error('kewarganegaraan')is-invalid @enderror" type="text" placeholder="Kewarganegaraan" id="kewarganegaraan" name="kewarganegaraan" value="{{ old('kewarganegaraan') }}" readonly>

            </div>
            <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
            <button type="submit" class="btn btn-primary" id="t_storePenduduk">Simpan</button>
            </div>
            </form>
        </div>
        </div>
    </div>
{{-- endmodalEditDataPenduduk --}}

<script>
    let t_editPenduduk = document.querySelectorAll("#t_editPenduduk");

    t_editPenduduk.forEach(t => {
        t.addEventListener('click', function(){
            let formEditPenduduk  = document.querySelector('.formEditPenduduk')
            let nokk              = formEditPenduduk.querySelector('#nokk')
            let nik               = formEditPenduduk.querySelector('#nik')
            let nama              = formEditPenduduk.querySelector('#nama')
            let jenisKelamin      = formEditPenduduk.querySelector('#jenisKelamin')   
            let alamat            = formEditPenduduk.querySelector('#alamat')
            let tempatLahir       = formEditPenduduk.querySelector('#tempatLahir')
            let tanggalLahir      = formEditPenduduk.querySelector('#tanggalLahir')
            let rt                = formEditPenduduk.querySelector('#rt')
            let rw                = formEditPenduduk.querySelector('#rw')
            let agama             = formEditPenduduk.querySelector('#agama')
            let statusPerkawinan  = formEditPenduduk.querySelector('#statusPerkawinan')
            let statusKeluarga    = formEditPenduduk.querySelector('#statusKeluarga')
            let golonganDarah     = formEditPenduduk.querySelector('#golonganDarah')
            let pekerjaan         = formEditPenduduk.querySelector('#pekerjaan')
            let kewarganegaraan   = formEditPenduduk.querySelector('#kewarganegaraan')

            fetch('penduduk/' + this.dataset.id +'/edit')
                .then(response => response.json())
                .then(data => {
                    let penduduk            = data.penduduk;
                    let formEditPenduduk    = document.querySelector(".formEditPenduduk");

                    formEditPenduduk.setAttribute('action', `/penduduk/${data.penduduk.id}`)

                    nokk.value             = penduduk.nokk;
                    nik.value              = penduduk.nik;
                    nama.value             = penduduk.nama;
                    jenisKelamin.value     = penduduk.jenisKelamin;
                    alamat.value           = penduduk.alamat;
                    tempatLahir.value      = penduduk.tempatLahir;
                    tanggalLahir.value     = penduduk.tanggalLahir;
                    rt.value               = penduduk.rt;
                    rw.value               = penduduk.rw;
                    agama.value            = penduduk.agama;
                    statusPerkawinan.value = penduduk.statusPerkawinan;
                    statusKeluarga.value   = penduduk.statusKeluarga;
                    golonganDarah.value    = penduduk.golonganDarah;
                    pekerjaan.value        = penduduk.pekerjaan;
                    kewarganegaraan.value  = penduduk.kewarganegaraan;
                });
        });
    });
</script>